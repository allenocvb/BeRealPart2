...
class PostCell: UITableViewCell {
    ...
    func configure(with post: Post) {
        ...
        let diffHours = Calendar.current.dateComponents([.hour], from: postCreatedDate, to: lastPostedDate).hour {
            ...
            // Hide the blue view if the given post was created within 24 hours of the current user's last post. (before or after)
            blurView.isHidden = abs(diffHours) < 24
        }else {
            
            // Default to blue if we can't get or compute the date's above for some reason
